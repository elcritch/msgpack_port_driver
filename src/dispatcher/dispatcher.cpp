...
response dispatcher::dispatch_call(RPCLIB_MSGPACK::object const &msg,
    ...
    if (it_func != end(funcs_)) {
        ...
        try {
            ...
            return response::make_error(
                id, clmdep_fmt::format("rpclib: {}", e.what()));
            ...
            return response::make_error(
                id,
                clmdep_fmt::format("rpclib: function '{0}' (called with {1} "
                                   "arg(s)) "
    ...
    return response::make_error(
        id, clmdep_fmt::format("rpclib: server could not find "
...
void dispatcher::enforce_unique_name(std::string const &func) {
    ...
    if (pos != end(funcs_)) {
        throw std::logic_error(
            clmdep_fmt::format("Function name already bound: '{}'. "
